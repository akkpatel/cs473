		     ...
		     Index *ind)
{
  /*!!!!! Implement log TF and IDF weighting scheme !!!!!*/
	...
	double numDocsContain=(double)ind->docCount(termID);
	double calc=log(totalNumDocs/(numDocsContain+1));
	return log(docTermFreq)*calc*qryTermWeight;
}
		     ...
		     Index *ind)
{
  /*!!!!! Implement customized weighting scheme !!!!!*/
	/*Probabilistic Model ---- http://orion.lcg.ufrj.br/Dr.Dobbs/books/book5/chap14.htm */
	double len=(double)ind->docLength(docID);
	double avgLen=(double)ind->docLengthAvg();
	double totalNumDocs=(double)ind->docCount();
	double numDocsContain=(double)ind->docCount(termID);
	/*double solve1 = log(totalNumDocs/numDocsContain);
	double solve2 = (1.2+1)*docTermFreq;
	double temp = (1-0.75)+(0.75*(len/avgLen));
	double solve3 = solve2/(1.2*temp*docTermFreq);
	double solve4 = (3+qryTermWeight)/(2+qryTermWeight);
	return solve1*(solve3+1)*solve4;*/
	/*double solve = (1-0.40+0.40*(len/avgLen));
	double solve1 = docTermFreq/solve;
	double solve2 = (3+1)*solve1/((3*(solve)+solve1)+1);
	double solve4 = (10+qryTermWeight)/(7+qryTermWeight);
	return solve2;*/
	return log((docTermFreq/qryTermWeight)/(numDocsContain/totalNumDocs));
}
